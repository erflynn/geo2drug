...
smok %>% write_csv("data/sample_lists/smoking_data.csv")
...
smok_gpl_meta %>% filter(bad & !downloaded) %>% select(gpl, title, description) # 2 possible bad
# GPL16956 - lncRNA, only probe seq
# GPL6955  - miRNA

smok_gpl_meta %>% filter(!downloaded & !bad) %>% select(gpl, title) # 4 failed to download that we do not expect
# GPL23159 - this should work
# GPL2829 - this should work
# GPL2986 - this should work
# GPL4133 - this should work

smok_gpl_meta %>% filter(downloaded & (missing | err)) %>% select(gpl, title) # 1 downloaded but missing?
# GPL18044 - miRNA
...
smok_ale <- left_join(dedup_smok2 %>% select(-organism), 
                       ale_dat %>% select(gse, gsm, text_sex, text_tissue_name), 
                       by=c("gse", "gsm")) # 12418
...
# add in the title
smok_metadat <- smok %>% select(gse, title, pubmed_id, submission_date,overall_design, summary)
...
summarized <- all_summary_dat %>% 
  ...
  mutate(study_type= case_when(
   ...
   dplyr::select(gse, title, pubmed_id, submission_date, overall_design, gpl, 
         ...
         study_type, summary)
summarized %>% write_csv("data/smok_dat/smoking_data_summary2.csv")
