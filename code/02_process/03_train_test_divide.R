...
list2 <- list.files("../../data/keep_labels/")

# read in the files
all_dat <- lapply(1:length(list2), function(i){
  f1 <- read.csv(sprintf("../../data/keep_labels/%s", list2[[i]]))
...
gse_info <- read_csv("../../data/sample_lists/gse_for_silver_std_human.csv")
...
# todo - do I need an extra step first here?

all_dat2 %>% 
  filter(gse %in% training$gse) %>%
  mutate(gse=unlist(gse)) %>%
  select(gse, gsm, text_sex) %>% 
  write_csv("../../data/sample_lists/human_training.csv")

all_dat2 %>% 
  filter(gse %in% testing$gse) %>% 
  mutate(gse=unlist(gse)) %>%
  select(gse, gsm, text_sex) %>% 
  write_csv("../../data/sample_lists/human_testing.csv")
