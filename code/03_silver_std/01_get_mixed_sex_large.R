...
gsm_filt_w_gse <- read.csv("data/sample_lists/gse_gsm_all_geo_dedup.csv")
gse_gsm <- inner_join(gse_no_cell, gsm_filt_w_gse[,c("gse", "gsm")])
length(unique(gse_gsm$gsm))

# mouse + rat
mouse_gse_counts <- read.csv("data/sample_lists/mouse_gse_counts.csv")
...
rat_gse_counts <- read.csv("data/sample_lists/rat_gse_counts.csv")
