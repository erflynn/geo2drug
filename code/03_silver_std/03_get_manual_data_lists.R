...
manual_w_gse %>% select(gse) %>% unique() %>% dplyr::count() # 356 studies
...
gpls2 <-gpls %>% group_by(gse) %>% dplyr::summarize(gpl=paste(gpl, collapse="|")) %>% ungroup()
...
# // TODO: get actual counts per study
