...
# add organism annotations
tail(comb_c_b)

require("GEOmetadb")
gses_list <- comb_c_b$GSE
formatted_list_gses <- paste(gses_list, collapse="\', \'")
...
require('tidyverse')
comb_study <- inner_join(comb_c_b, study_mapping, by=c("GSE"="gse"))

# add in the study counts if they're a drug study
...
study_w_counts <- inner_join(comb_study, mrh_counts, by=c("GSE"="gse"))
head(study_w_counts)
filter(study_w_counts, organism %in% c("Homo sapiens", "Mus musculus", "Rattus norvegicus"))
### PROBLEM - need to map all the data to drugbank annotations (or other?)
comb_c_b2 <- read.delim("data/hc_combined_df_drugbank.txt")
write.csv(study_w_counts[,c("GSE", "study_type", "organism.y", "source")], row.names=FALSE, file="combined_study_counts.csv")
# separate this out
